...
import { unlockPactClientSide } from "@/lib/modes/pact";
...
export function PactUnlockPanel({
  cid,
  partyCountN,
}: {
  cid: string;
  ...
  const [shares, setShares] = useState<string[]>(
    Array.from({ length: partyCountN }, () => ""),
  );

  const filledCount = shares.filter((s) => s.trim().length > 0).length;
  const allFilled = filledCount === partyCountN;
  const canCombine = phase.kind === "collecting" && allFilled;
  ...
  async function combineAndDecrypt() {
    ...
    try {
      const decoded = await unlockPactClientSide({
        cid,
        shareStrings: shares.map((s) => s.trim()),
      });
      ...
      setPhase({
        kind: "error",
        message:
          err instanceof Error ? err.message : "Failed to combine shares",
      });
    }
  }
      ...
      <div className="border-border/70 bg-muted/30 rounded-xl border p-5">
        <div className="mb-4 flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold">
            Unlock with all {partyCountN} shares
          </h2>
        </div>
        <p className="text-muted-foreground mb-4 text-sm leading-relaxed">
          Every party must paste their share below. When all{" "}
          <span className="text-foreground font-medium">{partyCountN}</span> are
          present, the browser combines them and decrypts the content. Shares
        ...
        <div className="mt-3 flex items-center justify-end">
          <p className="text-muted-foreground text-xs">
            {filledCount} of {partyCountN} present
          </p>
        </div>
      </div>
